...
func (m Model) viewDashboard(height int) string {
	...
	rightContent := m.renderDashboardDetails(rightWidth-2, height-2)
...
func (m Model) renderDashboardDetails(width, height int) string {
	...
	docContent := m.renderDashboardDoctor(width-4, topHeight-2)
	...
	actContent := m.renderDashboardQuickActions(width-4, bottomHeight-2)
...
func (m Model) renderDashboardDoctor(width, height int) string {
	...
	for _, c := range m.doctorReport.Containers {
		if c.Severity != models.SeverityPass {
			...
			builder.WriteString(fmt.Sprintf("  [%s] Container %s: %s\n", statusStyled, c.Name, msg))
			count++
		}
	}
	...
	for _, mt := range m.doctorReport.Mounts {
		if mt.Severity != models.SeverityPass {
			...
			builder.WriteString(fmt.Sprintf("  [%s] Mount %s: %s\n", statusStyled, mt.Target, mt.Issue))
			count++
		}
	}
	...
	for _, pt := range m.doctorReport.Ports {
		if pt.Severity != models.SeverityPass {
			...
			builder.WriteString(fmt.Sprintf("  [%s] Port conflict %d: %s\n", statusStyled, pt.Port, pt.Note))
			count++
		}
	}
...
func (m Model) viewContainers(height int) string {
	...
	case 0, 1: // Stacks
		...
		leftBox := leftStyle.Width(leftWidth).Height(innerContentHeight).Render(leftContent)
		...
		rightBox := rightStyle.Width(rightWidth).Height(innerContentHeight).Render(rightContent)
