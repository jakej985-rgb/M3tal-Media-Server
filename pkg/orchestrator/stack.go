...
func BuildRuntimeEnv(stackName string) (string, error) {
	globalPath := "/etc/m3tal/global.env"
	stackPath := fmt.Sprintf("/etc/m3tal/stacks/%s.env", stackName)
...
func (c *StackConfig) MergeVars() map[string]string {
	...
	globals := LoadEnvFile("/etc/m3tal/global.env")
	stackEnv := LoadEnvFile(fmt.Sprintf("/etc/m3tal/stacks/%s.env", c.Stack.Name))
	for k, v := range globals {
		merged[k] = v
	}
	for k, v := range stackEnv {
